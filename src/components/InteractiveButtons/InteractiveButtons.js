...
class InteractiveButtons extends Component {
  ...
  render() {
    return (
      <>
        <span className={this.props.open === false ? 'icons--hide' : null }>
          <span onClick={this.close} className={this.state.about === false ? 'dot dot--red' : 'dot dot__about dot--red'}><span className="sign sign--red">x</span></span>
          <span onClick={this.props.minimizeTerminal} className={this.state.about === false ? 'dot dot--yellow' : 'dot dot__about dot--yellow dot__about--yellow'}><span className="sign sign--yellow">-</span></span>
          <span onClick={this.props.maximizeTerminal} className={this.state.about === false ? 'dot dot--green' : 'dot dot__about dot--green'}>
  ...
  close = () => {
    ...
    currentRoute === '/about' ? this.props.history.push('/') : this.props.closeTerminal() ;
  }
}
